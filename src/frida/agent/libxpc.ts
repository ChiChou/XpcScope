...
const xpc_connection_create_mach_service = new NativeFunction(libxpc.getExportByName('xpc_connection_create_mach_service'), 'pointer', ['pointer', 'pointer', 'int']);
const xpc_connection_set_event_handler = new NativeFunction(libxpc.getExportByName('xpc_connection_set_event_handler'), 'void', ['pointer', 'pointer']);
const xpc_connection_resume = new NativeFunction(libxpc.getExportByName('xpc_connection_resume'), 'void', ['pointer']);

// connection info
const xpc_connection_get_name = new NativeFunction(libxpc.getExportByName('xpc_connection_get_name'), 'pointer', ['pointer']);
const xpc_connection_get_pid = new NativeFunction(libxpc.getExportByName('xpc_connection_get_pid'), 'int', ['pointer']);

const xpc_copy_description = new NativeFunction(libxpc.getExportByName('xpc_copy_description'), 'pointer', ['pointer']);
const xpc_get_type = new NativeFunction(libxpc.getExportByName('xpc_get_type'), 'pointer', ['pointer']);

// buffer types
const xpc_string_get_string_ptr = new NativeFunction(libxpc.getExportByName('xpc_string_get_string_ptr'), 'pointer', ['pointer']);
const xpc_string_get_length = new NativeFunction(libxpc.getExportByName('xpc_string_get_length'), 'size_t', ['pointer']);
const xpc_data_get_bytes_ptr = new NativeFunction(libxpc.getExportByName('xpc_data_get_bytes_ptr'), 'pointer', ['pointer']);
const xpc_data_get_length = new NativeFunction(libxpc.getExportByName('xpc_data_get_length'), 'size_t', ['pointer']);
const xpc_uuid_get_bytes = new NativeFunction(libxpc.getExportByName('xpc_uuid_get_bytes'), 'pointer', ['pointer']);

// primitive types
const xpc_double_get_value = new NativeFunction(libxpc.getExportByName('xpc_double_get_value'), 'double', ['pointer']);
const xpc_bool_get_value = new NativeFunction(libxpc.getExportByName('xpc_bool_get_value'), 'bool', ['pointer']);
const xpc_int64_get_value = new NativeFunction(libxpc.getExportByName('xpc_int64_get_value'), 'int64', ['pointer']);
const xpc_uint64_get_value = new NativeFunction(libxpc.getExportByName('xpc_uint64_get_value'), 'uint64', ['pointer']);

// apply functions
const xpc_dictionary_apply = new NativeFunction(libxpc.getExportByName('xpc_dictionary_apply'), 'bool', ['pointer', 'pointer']);
const xpc_array_apply = new NativeFunction(libxpc.getExportByName('xpc_array_apply'), 'bool', ['pointer', 'pointer']);

// remote resources
const xpc_fd_dup = new NativeFunction(libxpc.getExportByName('xpc_fd_dup'), 'int', ['pointer']);

const xpcTypes = libxpc.enumerateExports().filter(s => s.name.startsWith('_xpc_type_'));
const xpcDictionaryType = xpcTypes.find(s => s.name === '_xpc_type_dictionary')!.address;
const xpcArrayType = xpcTypes.find(s => s.name === '_xpc_type_array')!.address;

const fcntl = new NativeFunction(Module.findExportByName(null, 'fcntl')!, 'int', ['int', 'int', 'pointer']);
...
class LogSerializer {
  ...
  private dump(xpcObj: NativePointer) {
    ...
    if (t.equals(xpcDictionaryType)) { // dictionary?
      const keys: string[] = [];
      const values: XPCNode[] = [];

      xpc_dictionary_apply(xpcObj, new ObjC.Block({
        retType: 'bool',
    ...
    if (t.equals(xpcArrayType)) { // array?
      const values: XPCNode[] = [];
      xpc_array_apply(xpcObj, new ObjC.Block({
        retType: 'bool',
    ...
    for (const xpcType of xpcTypes) {
      if (xpc_get_type(xpcObj).equals(xpcType.address)) {
        ...
        if (type === 'string') {
          const value = xpc_string_get_string_ptr(xpcObj)
            .readUtf8String(xpc_string_get_length(xpcObj).toNumber());
...
export async function start() {
  const dispacher = DebugSymbol.getFunctionByName('_xpc_connection_call_event_handler');
  ...
  for (const suffix of ['', '_with_reply', '_with_reply_sync']) {
    ...
    Interceptor.attach(func, {
      onEnter(args) {
        ...
        const backtrace = Thread.backtrace(this.context, Backtracer.ACCURATE).map(DebugSymbol.fromAddress).map(String);
