...
function loadFramework() {
  ...
  for (const prefix of prefixes) {
    const bundle = ObjC.classes.NSBundle.bundleWithPath_(
      `/System/Library/${prefix}Frameworks/ServiceManagement.framework`,
    );
...
export function jobs(domain: SMDomain) {
  ...
  const SMCopyAllJobDictionaries = new NativeFunction(
    sm.findExportByName("SMCopyAllJobDictionaries")!,
    "pointer",
    ["pointer"],
  );
